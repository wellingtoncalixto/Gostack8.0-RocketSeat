...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    if (provider_id === req.userId) {
      return res
        .status(401)
        .json({ error: 'You can not create appointment with your self' });
    }
  ...
  async delete(req, res) {
    const appointment = await Appointmente.findByPk(req.params.id, {
      include: [
        {
          model: User,
          as: 'provider',
    ...
    await Mail.sendMail({
      ...
      text: `O cliente ${appointment.provider.name} cancelou o gendamento do dia ${appointment.date}`,
    });
